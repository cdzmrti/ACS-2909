window.onload= function(){
    ...
    button.addEventListener("click", function(){
        ...
        xhr.open('GET', 'assignment4.json');//local file
        xhr.onload = function(){
...
function process(myObject){
    ...
    for(const obj of objCopy){//iterate over the objects
        var txt = "";
        for (const key in obj){//iterate over the properties
            if (key !== "Botanical Name")
               txt += key + ": "+ obj[key] + "<br> <br>";
        }//inner for loop
        createNote(txt);
    }// outer for loop
}
